...
if ( ! class_exists( 'BSF_AIOSRS_Pro_Schema_Wizard' ) ) :
	...
	class BSF_AIOSRS_Pro_Schema_Wizard {
		...
		public function choose_schema_type() {
			?>
			<h1><?php _e( 'Select the Schema Type You Need to Add:', 'wp-schema-pro' ); ?></h1>
			<form method="post">
				...
				<p class="aiosrs-pro-setup-actions step">
					<input type="submit" class="uct-activate button-primary button button-large button-next" disabled="true" value="<?php _e( 'Next', 'wp-schema-pro' ); ?>" name="save_step" />
		...
		public function implement_on_callback() {
			...
			printf(
				...
				__( 'Where <i>%s</i> schema should be integrated?', 'wp-schema-pro' ),
				$title
			);
			?>
				</h1>
			...
			<div class="aiosrs-pro-setup-next-steps">
				<div class="aiosrs-pro-setup-next-steps-last">
					...
					<p class="success">
						...
						printf(
							/* translators: 1 schema title */
							__( 'Congratulations! The <i>%s</i> schema has been added and enabled on selected pages.', 'wp-schema-pro' ),
							$title
						);
						?>
					</p>
					<p class="success">
						<b><?php _e( 'Here is what you do after setup is complete:', 'wp-schema-pro' ); ?></b><br>
						<?php _e( 'Step 1: Check the Schema you just created.', 'wp-schema-pro' ); ?><br>
						<?php if ( 'article' == $schema_type || 'course' == $schema_type ) { ?>
							<?php _e( 'Step 2: Test if Schema is integrated correctly.', 'wp-schema-pro' ); ?>
						<?php } else { ?>
							<?php _e( 'Step 2: Add necessary Schema information on individual pages and posts.', 'wp-schema-pro' ); ?><br>
							<?php _e( 'Step 3: Test if Schema is integrated correctly.', 'wp-schema-pro' ); ?>
						<?php } ?>
					</p>
					...
					<table class="form-table aiosrs-pro-schema-ready">
						<tr>
							<td scope="row" >
								<a href="<?php echo ( $schema_id ) ? get_edit_post_link( $schema_id ) : '#'; ?>" type="button" class="button button-primary button-hero" ><?php _e( 'Complete Setup', 'wp-schema-pro' ); ?></a>
							</td>
						</tr>
					</table>
